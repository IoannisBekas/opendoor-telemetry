...
## 0. What ships today, and how to replace it

The dashboard does **not** currently use Higgsfield video. It ships a native
canvas layer (`ambientGround()` in `dashboard/template.html`) that implements
Prompts **B** and **C** procedurally: an isometric parcel lattice with slowly
pulsing cobalt nodes, over soft architectural light drifting across a dark slate
field. It is a real asset — generated, animated, brand-matched — but it is drawn
in the page rather than rendered by a video model.

Why: driving Higgsfield requires an account login and spends credits, so the
assets have to be generated by the account holder, not the pipeline.

Measured against the live page (1400×900, composited over `#0A1020`):

| Metric | Value | Contract |
| :--- | :--- | :--- |
| Mean luminance | 0.62% | far under the band — see note |
| Peak luminance | 4.19% | no hotspots |
| Centre vs edge | 0.58% vs 0.65% | dead zone holds |
| Body text worst case | 10.17:1 | AAA |
| Muted text worst case | 4.63:1 | AA |
| Motion | ~1 px/s lattice drift | equivalent to 1–2 / 10 |

> **The luminance band is deliberately not met, and that is correct.** The
> 12–18% band in §1 assumes video sitting behind a `rgba(10,16,32,.55)` scrim.
> The canvas layer composites straight onto the ground with no scrim, so the
> same end-state contrast requires a far darker layer. Judge this layer on the
> text-contrast rows, not the band.

### Swapping in real Higgsfield output

The page reads its ground from one element, so the swap is local:

1. Generate Prompt C at 16:9, loop it per §5, and export `ambient-c.mp4`
   plus a `ambient-c-poster.webp` first frame.
2. Drop both next to `docs/index.html`.
3. Replace `<canvas id="bg">` with:

```html
<video id="bg" autoplay muted loop playsinline
       poster="ambient-c-poster.webp" aria-hidden="true"></video>
<script>
  // Honour reduced motion: poster only, no playback.
...
4. Add the scrim the video contract assumes — without it the §4 contrast
   figures do not hold:
...
5. Delete the `ambientGround()` IIFE from `template.html`.

Everything else — `#bg` sizing, the `z-index: 1` on `.wrap` and `.mast` — already
works for either element.
